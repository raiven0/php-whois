...
function lookUpDomain($domain){
	...
	if(!$whoIsServer) {
		return "HATA: ALAN ADINI SORGULAMAK ICIN KAYITLI SUNUCU BULUNAMADI" . "<br />";
	}
	$whoIsResult = getWhoisServerDetails($whoIsServer, $domain);
	if(!$whoIsResult) {
		return "HATA: SUNUCUDAN ALAN ADI ICIN YANIT ALINAMADI" . "<br />";
	} else {
		while(strpos($whoIsResult, "Whois Sunucusu:") !== FALSE){
			preg_match("/Whois Sunucusu: (.*)/", $whoIsResult, $matches);
			$secondary = $matches[1];
			if($secondary) {
	...
	return "<br><div class='row'><div class='col-lg-4'><hr></div><div class='col-lg-4'>DETAYLI WHOIS SONUCU</div><div class='col-lg-4'><hr></div></div>\n\n" . $whoIsResult;
}
...
function getWhoisServerDetails($whoIsServer, $domain) {
	$port = 43;
	$timeout = 10;
	$whoIsInfo = @fsockopen($whoIsServer, $port, $errno, $errstr, $timeout) or die("Hata " . $errno . " - " . $errstr);	
	...
	fclose($whoIsInfo);
	$whosIsResults = "";
	if((strpos(strtolower($output), "hata") === FALSE) && (strpos(strtolower($output), "Alan Adı Bulunamadı") === FALSE)) {
